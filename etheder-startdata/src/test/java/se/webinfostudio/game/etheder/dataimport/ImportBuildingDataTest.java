 ...
 *
 */
public class ImportBuildingDataTest {
	...
	void beforeEach() throws DataImportException {
		...
		when(readExcel.getExcelRows(EXCEL_FILE_NAME, "Buildings")).thenReturn(asList(row, row, row).iterator());

		sut = new ImportBuildingData(readExcel);
	}
